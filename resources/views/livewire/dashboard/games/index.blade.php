<div class="animate-fade-in space-y-8">

    {{-- 1. الهيدر: العنوان وزر الإضافة --}}
    <div class="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div>
            <h1 class="text-3xl font-extrabold text-slate-900 tracking-tight mb-2">
                مكتبة الألعاب
            </h1>
            <p class="text-slate-500">
                لديك <span class="font-bold text-emerald-600 font-sans mx-1">{{ $games->count() }}</span> لعبة منشورة. اللاعبون بانتظار تحديثك القادم! 🎮
            </p>
        </div>

        <a href="{{ route('dashboard.games.create') }}" class="group bg-slate-900 hover:bg-emerald-600 text-white px-6 py-3 rounded-xl text-sm font-bold transition-all duration-300 shadow-lg hover:shadow-emerald-500/30 flex items-center gap-2">
            <span class="bg-white/10 group-hover:bg-white/20 p-1 rounded-lg transition">
                <i class="fa-solid fa-cloud-arrow-up text-xs"></i>
            </span>
            <span>رفع لعبة جديدة</span>
        </a>
    </div>

    {{-- 2. شريط التحكم: البحث والفلتر --}}
    <div class="bg-white border border-slate-200 rounded-2xl p-2 shadow-sm flex flex-col md:flex-row gap-2">
        {{-- البحث --}}
        <div class="relative flex-1">
            <div class="absolute inset-y-0 right-0 flex items-center pr-4 pointer-events-none text-slate-400 group-focus-within:text-emerald-500 transition-colors">
                <i class="fa-solid fa-magnifying-glass"></i>
            </div>
            <input
                wire:model.live.debounce.300ms="search"
                type="text"
                placeholder="ابحث عن اسم اللعبة..."
                class="w-full bg-slate-50 hover:bg-white focus:bg-white border border-transparent focus:border-emerald-500 rounded-xl py-3 pr-11 pl-4 text-sm outline-none transition-all duration-300 placeholder:text-slate-400 font-medium"
            >
        </div>

        {{-- الفلتر --}}
        <div class="relative min-w-[180px]">
            <div class="absolute inset-y-0 right-0 flex items-center pr-4 pointer-events-none text-slate-400">
                <i class="fa-solid fa-filter"></i>
            </div>
            <select wire:model.live="filter_status" class="w-full appearance-none bg-slate-50 hover:bg-white focus:bg-white border border-transparent focus:border-emerald-500 rounded-xl py-3 pr-11 pl-4 text-sm text-slate-600 font-bold outline-none transition-all cursor-pointer">
                <option value="">كل الحالات</option>
                <option value="published">منشور (Published)</option>
                <option value="draft">مسودة (Draft)</option>
            </select>
            <div class="absolute inset-y-0 left-0 flex items-center pl-4 pointer-events-none text-slate-400">
                <i class="fa-solid fa-chevron-down text-xs"></i>
            </div>
        </div>
    </div>

    {{-- 3. شبكة الألعاب --}}
    <div class="grid grid-cols-1 gap-4">
        @forelse($games as $game)
            <div class="group bg-white border border-slate-200 rounded-2xl p-6 hover:border-emerald-500/30 hover:shadow-[0_8px_30px_rgb(0,0,0,0.06)] hover:-translate-y-1 transition-all duration-300 relative overflow-hidden">

                {{-- شريط جانبي ملون --}}
                <div class="absolute top-0 right-0 w-1 h-full bg-slate-100 group-hover:bg-emerald-500 transition-colors duration-300"></div>

                <div class="flex flex-col md:flex-row gap-6 pr-3">
                    {{-- الأيقونة أو صورة اللعبة --}}
                    <div class="shrink-0">
                        @if($game->thumbnail)
                            <img src="{{ asset('storage/' . $game->thumbnail) }}" alt="{{ $game->title }}" class="w-16 h-16 rounded-2xl object-cover border border-slate-100 group-hover:border-emerald-100 transition shadow-sm">
                        @else
                            <div class="w-16 h-16 rounded-2xl bg-slate-50 text-slate-400 group-hover:bg-emerald-50 group-hover:text-emerald-600 flex items-center justify-center transition-colors duration-300 border border-slate-100 group-hover:border-emerald-100">
                                <i class="fa-solid fa-gamepad text-2xl"></i>
                            </div>
                        @endif
                    </div>

                    {{-- المحتوى --}}
                    <div class="flex-1 min-w-0">
                        <div class="flex flex-wrap items-center justify-between gap-3 mb-2">
                            <div class="flex items-center gap-3">
                                <h3 class="text-lg font-extrabold text-slate-900 group-hover:text-emerald-600 transition-colors tracking-tight">
                                    {{ $game->title }}
                                </h3>
                                
                                {{-- حالة النشر --}}
                                <span class="px-2.5 py-0.5 rounded-full text-[10px] font-bold border flex items-center gap-1 uppercase tracking-wider
                                    {{ $game->is_published
                                        ? 'bg-emerald-50 text-emerald-700 border-emerald-100'
                                        : 'bg-amber-50 text-amber-700 border-amber-100' }}">
                                    @if($game->is_published)
                                        <i class="fa-solid fa-check-circle"></i> Published
                                    @else
                                        <i class="fa-solid fa-pencil"></i> Draft
                                    @endif
                                </span>
                            </div>

                            {{-- زر التعديل --}}
                            <a href="{{ route('dashboard.games.edit', $game) }}" class="opacity-0 group-hover:opacity-100 text-slate-400 hover:text-emerald-600 bg-slate-50 hover:bg-emerald-50 px-3 py-1.5 rounded-lg text-xs font-bold transition-all transform translate-x-2 group-hover:translate-x-0">
                                <i class="fa-solid fa-pen-to-square"></i> تعديل
                            </a>
                        </div>

                        <p class="text-slate-500 text-sm leading-relaxed mb-5 line-clamp-2 max-w-3xl">
                            {{ $game->description ?? 'لا يوجد وصف متاح لهذه اللعبة حالياً.' }}
                        </p>

                        {{-- الفوتر: المعلومات الإضافية --}}
                        <div class="flex items-center gap-6 text-xs text-slate-400 font-medium border-t border-dashed border-slate-100 pt-4">
                            
                            {{-- الإصدار --}}
                            <div class="flex items-center gap-2 text-slate-600 bg-slate-50 px-2 py-1 rounded-md">
                                <span class="w-2 h-2 rounded-full bg-indigo-500 shadow-[0_0_8px_rgba(99,102,241,0.5)]"></span>
                                <span class="font-mono">v{{ $game->version ?? '1.0' }}</span>
                            </div>

                            {{-- التحميلات --}}
                            <div class="flex items-center gap-1.5 hover:text-emerald-500 transition cursor-help" title="عدد التحميلات">
                                <i class="fa-solid fa-download text-emerald-400"></i>
                                <span class="font-sans font-bold">{{ number_format($game->downloads_count ?? 0) }}</span>
                            </div>

                            {{-- المشاهدات --}}
                            <div class="flex items-center gap-1.5 hover:text-blue-500 transition" title="المشاهدات">
                                <i class="fa-regular fa-eye text-blue-400"></i>
                                <span class="font-sans font-bold">{{ number_format($game->views_count ?? 0) }}</span>
                            </div>

                            {{-- السعر --}}
                            <div class="flex items-center gap-1.5">
                                @if($game->price > 0)
                                    <span class="text-slate-700 font-bold">{{ $game->price }} SAR</span>
                                @else
                                    <span class="text-emerald-600 font-bold">مجاني</span>
                                @endif
                            </div>

                            {{-- الوقت --}}
                            <div class="flex items-center gap-1.5 mr-auto" title="آخر تحديث">
                                <i class="fa-regular fa-clock"></i>
                                <span>{{ $game->updated_at->diffForHumans() }}</span>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        @empty
            {{-- حالة الفراغ --}}
            <div class="flex flex-col items-center justify-center py-16 bg-white border-2 border-dashed border-slate-200 rounded-2xl group hover:border-emerald-300 hover:bg-emerald-50/10 transition-colors duration-300">
                <div class="w-20 h-20 bg-slate-50 rounded-full flex items-center justify-center mb-4 shadow-sm text-slate-300 group-hover:text-emerald-500 group-hover:scale-110 transition duration-300">
                    <i class="fa-solid fa-gamepad text-4xl"></i>
                </div>
                <h3 class="text-lg font-bold text-slate-900 mb-1">لا توجد ألعاب بعد!</h3>
                <p class="text-slate-500 text-sm mb-6 text-center max-w-xs">
                    لم نتمكن من العثور على أي ألعاب. ابدأ برفع لعبتك الأولى وشاركها مع العالم.
                </p>
                @if($search)
                    <button wire:click="$set('search', '')" class="text-emerald-600 font-bold hover:underline text-sm">
                        مسح البحث والعودة
                    </button>
                @else
                    <a href="{{ route('dashboard.games.create') }}" class="bg-emerald-600 text-white px-6 py-2.5 rounded-xl font-bold text-sm hover:bg-emerald-700 hover:shadow-lg transition">
                        <i class="fa-solid fa-cloud-arrow-up ml-1"></i> رفع لعبتي الأولى
                    </a>
                @endif
            </div>
        @endforelse
    </div>

    {{-- ترقيم الصفحات --}}
    <div class="mt-8">
        {{ $games->links() }}
    </div>
</div>